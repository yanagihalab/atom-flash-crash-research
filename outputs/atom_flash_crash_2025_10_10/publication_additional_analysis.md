...
360窓は30日×12本の非重複2時間窓です。同時刻30窓は各日の20:30–22:30 UTCです。出来高・値幅・乖離・総流量は上側片側、符号付き純流入は対照中央値からの絶対偏差による両側検定です。p値は `(1 + exceedances)/(N + 1)`。暗号資産時系列の直列依存により、交換可能性は厳密には保証されないため、p値は記述的なランダム化推論として解釈します。
...
| spot_trade_activity → spot_range_stress | 5 | 0.116 | 0.074 | 0.3609 | 235 |
...
| x → y | 最大|r|ラグ (分) | r | lag 0 r | 循環シフト p | n |
|---|---:|---:|---:|---:|---:|
| confirmed_exchange_in_atom → spot_range_stress | -60 | 0.027 | 0.015 | 0.0620 | 8,916 |
| confirmed_exchange_out_atom → spot_range_stress | -60 | 0.040 | 0.028 | 0.0002 | 8,916 |
| unconfirmed_behavioral_in_atom → spot_range_stress | -25 | 0.028 | 0.017 | 0.1856 | 8,923 |
| all_behavioral_candidate_in_atom → spot_range_stress | -60 | 0.030 | 0.019 | 0.2975 | 8,916 |
| confirmed_exchange_net_in_atom → spot_return | -25 | 0.025 | 0.016 | 0.4207 | 8,923 |
| ibc_inbound_atom → spot_range_stress | -60 | 0.042 | 0.027 | 0.0034 | 8,916 |
| ibc_outbound_atom → spot_range_stress | -20 | 0.043 | 0.034 | 0.0142 | 8,924 |
| ibc_net_inbound_atom → spot_return | -5 | 0.066 | 0.063 | 0.0030 | 8,927 |
...
| x → y | 最大|r|ラグ (分) | r | lag 0 r | 循環シフト p | n |
|---|---:|---:|---:|---:|---:|
| confirmed_exchange_in_atom → spot_range_stress | 40 | -0.571 | 0.088 | 0.0417 | 40 |
| confirmed_exchange_out_atom → spot_range_stress | -40 | -0.310 | 0.037 | 0.0417 | 40 |
| unconfirmed_behavioral_in_atom → spot_range_stress | 45 | -0.506 | -0.007 | 0.0417 | 39 |
| all_behavioral_candidate_in_atom → spot_range_stress | 45 | -0.522 | 0.001 | 0.0417 | 39 |
| confirmed_exchange_net_in_atom → spot_return | -40 | 0.243 | 0.009 | 0.0417 | 40 |
| ibc_inbound_atom → spot_range_stress | 35 | -0.383 | -0.003 | 0.6250 | 41 |
| ibc_outbound_atom → spot_range_stress | 35 | -0.302 | 0.071 | 0.8750 | 41 |
| ibc_net_inbound_atom → spot_return | -30 | 0.323 | 0.131 | 0.0417 | 42 |

正のラグは x が y に先行する定義です。全報告ラグから最大絶対相関を選ぶため、循環シフト検定では各置換について同じく最大絶対相関を計算し、多重ラグ探索を反映しています。局所窓は標本数が小さく、因果方向を確定する検定ではありません。

1分の主要価格系列は最大相関がすべて lag 0 にあり、分単位の安定した市場間先行は示しません。5分の全標本では最大でも |r|=0.066 と小さく、確認済み取引所流入→市場ストレスは r=0.027、p=0.0620 でした。局所4時間の同系列は lag 40分、r=-0.571、p=0.0417 ですが、符号・ラグが全標本と一致せず、p=0.0417 は 23 シフトにおける解像度下限です。したがって、頑健なオンチェーン先行の証拠とは解釈しません。
